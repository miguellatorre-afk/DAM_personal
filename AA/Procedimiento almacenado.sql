...
CREATE OR REPLACE PROCEDURE COMPROBAR(SEGURO IN VEHICULOS.ASEGURADO%TYPE , N_VEHICULO OUT NUMBER) AS
...
OU_MAT VEHICULOS.MATRICULA%TYPE;
OU_MARCA VEHICULOS.MARCA%TYPE;
OU_MODELO VEHICULOS.MODELO%TYPE;
...
OPEN C1;
LOOP
   FETCH C1 INTO OU_MAT, OU_MARCA, OU_MODELO;
   EXIT WHEN C1%NOTFOUND;
    DBMS_OUTPUT.PUT_LINE('MATRICULA     ' || OU_MAT);
    DBMS_OUTPUT.PUT_LINE('MARCA     ' || OU_MARCA);
    DBMS_OUTPUT.PUT_LINE('MODELO    ' || OU_MODELO);
...
BEGIN
    COMPROBAR('S', RESULTS);
    DBMS_OUTPUT.PUT_LINE('TOTAL DE VEHICULOS ' || RESULTS);
END;
/
